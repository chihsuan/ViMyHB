...
public class FirstLoginActivity extends ActionBarActivity {
        ...
        button.setOnClickListener(new View.OnClickListener() {
            public void onClick(View view) {
                ...
                if (User.userID.length() == 4 && User.password.length() > 0) {
                    postLogin(User.userPhoneNumber, User.userID, User.password);
                    ...
                    if (User.isLogin) {
                        Toast.makeText(view.getContext(), "登入成功",
                                Toast.LENGTH_SHORT).show();


                        Intent refresh = new Intent(FirstLoginActivity.this, MainActivity.class);
                        startActivity(refresh);
                        finish();
                    }
                    else {
                        Toast.makeText(view.getContext(), "登入失敗，請重新輸入正確帳戶資訊",
                                Toast.LENGTH_SHORT).show();
                    }
                }
                else {
    ...
    private void postLogin (String userPhoneNumber, String userID, String password) {
